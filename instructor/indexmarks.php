<?php include('../Partial/instructor-header.php') ?>


<div class="container">
    <div class="Admin-Content">
        <h3>Update Marks</h3>
    </div>

    <div class="Add-admin">
            <a href="addmarks.php" class="btn btn-success">Add Marks</a>            
    </div>
</div>

    <br>


   <div class="container table-responsive">
  <table class="col-12 table">
    
      <tr>
        <th scope="col">ID</th>
        <th scope="col">RollNo</th>
        <th scope="col">TestID</th>
        <th scope="col">Description</th>
        <th scope="col">Date</th>        
        <th scope="col">Marks</th>                   
        <th scope="col">Action</th>        
      </tr>
    
    

  <?php

      //GET QUERY THE ADMIN
      $sql = "SELECT * FROM `monmarks` WHERE `TeacherID`= '$_SESSION[ID]'";

      // EXCUTE THE QUERY
      $res = mysqli_query($conn, $sql);

      // checking the query excute or not
      if($res==TRUE)
      {
        //count rows
        $rows = mysqli_num_rows($res); //get all the row in the database

        $No=1;

        // check num of rows
        if($rows>0)
        {
          // we have data in database
          while($rows = mysqli_fetch_assoc($res))
          {
            // using while loop to get data

            // get indivisual data
            $ID = $rows['ID'];
            $RollNo = $rows['RollNo'];
            $TestID = $rows['TestID'];
            $Desc = $rows['Desc'];
            $Date = $rows['Date'];
            $Marks = $rows['Marks'];

            // display the values in our tables
            ?>
    
      <tr>
        <th scope="row"><?php echo $No++; ?></th>
        <td><?php echo $RollNo; ?></td>        
        <td><?php echo $TestID; ?></td>
        <td><?php echo $Desc; ?></td>
        <td><?php echo $Date; ?></td>               
        <td><?php echo $Marks ?></td>        
        <td>
          <a href="updatemarks.php?ID=<?php echo $rows['ID'];?>" class="btn btn-primary">Edit</a>        
        </td>
      </tr>

            <?php

          }
        }
        else{
                echo "<script>
                      alert('You not add student progress')                      
                      </script>";
        }

      }
  ?>

</table>
</div>

<br>
<?php include('../Partial/instructor-footer.php') ?>
